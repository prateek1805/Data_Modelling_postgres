...
def process_customer_data(spark,input_data,output_data):
    ...
    bank_file_count = Bank_df.count()                        # taking the count of the file
    ...
    Customer_count = Customer.count()        # taking the count of customer updated to table from file
    
    if(bank_file_count != Customer_count):    #Checking if the no of rows are same in customer table and bank file
        print("Customer Count doesn't match")
        exit()
    ...
    if(bank_file_count != Account_count):
        print("Account Count doesn't match")
        exit()
...
def process_transaction_data(spark,input_data,output_data):
    ...
    df = spark.read.csv(os.path.join(input_data+"Bank.csv"),header = True,inferSchema = True)
